...
static char			ifilename[_MAX_PATH];

static char			szfilenamei[_MAX_PATH];

static char			szOutputDir[_MAX_PATH];

static char			szfilenameLog[_MAX_PATH] = "CAQ2WAV_log.txt";

unsigned char		g_cflOutputDir = 0;

unsigned char		g_cflAppendEnable = 0;
...
char    g_cflInvertLevel = 0; //1= invert output level
char    g_cflSine = 1;      //1= sine wave output, 0 = square wave output
...
#define NUMSAMPLES_SILENCE_11K   9
#define NUMSAMPLES_HALFSHORT_11K 3
#define NUMSAMPLES_HALFLONG_11K  6

#define NUMSAMPLES_SILENCE_10K   9
#define NUMSAMPLES_HALFSHORT_10K 3
#define NUMSAMPLES_HALFLONG_10K  6

#define NUMSAMPLES_SILENCE_7K   6
#define NUMSAMPLES_HALFSHORT_7K 2
#define NUMSAMPLES_HALFLONG_7K  4

#define NUMSAMPLES_SILENCE_3K   3
#define NUMSAMPLES_HALFSHORT_3K 1
#define NUMSAMPLES_HALFLONG_3K  2


#define SPEED_44K 44100
#define SPEED_22K 22050
#define SPEED_11K 11025
#define SPEED_10K 10800
#define SPEED_7K 7200
#define SPEED_3K 3900
...
int init(int argc, char *argv[])
{
	...
	for (i = 1; i < argc - 1; i++)
	{
		...
		if (strcmp(argv[i], "-a") == 0) {
			...
			g_nSamplesPerSecond = SPEED_11K;
			g_nSamples_Silence = NUMSAMPLES_SILENCE_11K;
			g_nSamples_HalfShort = NUMSAMPLES_HALFSHORT_11K;
			g_nSamples_HalfLong = NUMSAMPLES_HALFLONG_11K; 	   	   	    	      	           	      	    
		}
		else if (strcmp(argv[i], "-f10") == 0) {
			g_nSamplesPerSecond = SPEED_10K;
			g_nSamples_Silence = NUMSAMPLES_SILENCE_10K;
			g_nSamples_HalfShort = NUMSAMPLES_HALFSHORT_10K;
			g_nSamples_HalfLong = NUMSAMPLES_HALFLONG_10K; 	   	   	    	      	           	      	    
		}
		else if (strcmp(argv[i], "-f7") == 0) {
			g_nSamplesPerSecond = SPEED_7K;
			g_nSamples_Silence = NUMSAMPLES_SILENCE_7K;
			g_nSamples_HalfShort = NUMSAMPLES_HALFSHORT_7K;
			g_nSamples_HalfLong = NUMSAMPLES_HALFLONG_7K; 	  	  	   	         	          	     	   
		}
		else if (strcmp(argv[i], "-f3") == 0) {
			g_nSamplesPerSecond = SPEED_3K;
			...
			g_cflSine = 1;      //1= sine wave output, 0 = square wave output		
		}
		else if (strcmp(argv[i], "-p") == 0) {
			g_cflOutputDir = 1;
			strcpy(szOutputDir, argv[i+1]);      	 	 
		}	   	   	     	 
...
int main(int argc, char *argv[])
{
	...
	if (init(argc, argv))
	{  
		printf("Usage: %s [-a] [-f44|-f22|-f11|-f10|-f7|-f3] [-l] [-n|-q] <.CAQ inputfile>\n", argv[0]);
		printf("Options: -a append options to output filename\n");
		printf("         -f44 change sampling freq to 44100 Hz\n");
		printf("         -f22 change sampling freq to 22050 Hz\n");
		printf("         -f11 change sampling freq to 11025 Hz\n");
		printf("         -f10 force sampling freq to 10800 Hz\n");
		printf("         -f7 force sampling freq to 7200 Hz\n");
		printf("         -f3 force sampling freq to 3900 Hz\n");			  	    	  	    		
		printf("         -l output to log file\n");
		printf("         -p <prefixordir>\n");
		printf("         -n force sin(x) output waveform\n");			  	  	  
		printf("         -q force square output waveform\n");	    	
		exit(1);
	}

	strcpy(szfilenamei, argv[argc - 1]);
	MakeOutputWave();
...
unsigned char squareandCheckInv(int val) {
	...
	if (g_cflInvertLevel) {
		...
		if ((unsigned char)val > 127) return 0;
		else return 255;
	
	
	} else {
	
		if ((unsigned char)val > 127) return 255;
		else return 0;
	}
}
...
unsigned char CheckInv(int val) {
	...
	if (g_cflInvertLevel) return (255-(unsigned char)val);
...
void    SetupOutputWavelengths(void)
{
    ...
    if (g_cflSine)
    {
        ...
        for (j = 0; j < g_nSamples_HalfShort; j++)
        {
			...
			value = WAV_SILENCE - (unsigned char)( (int)( (double)WAV_A*sin(  ((double)k+fwidthfudgei) * PI / ((double)g_nSamples_HalfShort + fwidthfudge) )  )   );
			g_cbufShortFullWave[j] = CheckInv(value);
        }
        ...
        for (j = 0; j < g_nSamples_HalfLong; j++)
        {
			...
			value = WAV_SILENCE - (unsigned char)( (int)( (double)WAV_A*sin(  ((double)k+fwidthfudgei) * PI / ((double)g_nSamples_HalfLong + fwidthfudge) ) ) );
			g_cbufLongFullWave[j] = CheckInv(value);
        }
    ...
    else
    {
        ...
        for (j = 0; j < g_nSamples_HalfShort; j++) {
			...
			k = j - g_nSamples_HalfShort;
			value = WAV_SILENCE - (unsigned char)( (int)( (double)WAV_A*sin(  ((double)k+fwidthfudgei) * PI / ((double)g_nSamples_HalfShort + fwidthfudge) )  )   );
			g_cbufShortFullWave[j] = squareandCheckInv(value);
        }
        ...
        for (j = 0; j < g_nSamples_HalfLong; j++) {
			...
			k = j - g_nSamples_HalfLong;
			value = WAV_SILENCE - (unsigned char)( (int)( (double)WAV_A*sin(  ((double)k+fwidthfudgei) * PI / ((double)g_nSamples_HalfLong + fwidthfudge) ) ) );
			g_cbufLongFullWave[j] = squareandCheckInv(value);
        }
        for (j = g_nSamples_HalfLong; j < 2 * g_nSamples_HalfLong; j++) {
            //g_cbufLongFullWave[j] = WAV_H;
			k = j;
...
void    MakeOutputWave(void)
{
    ...
    if (pfile)
    {
		...
		if (g_cflOutputDir) {
			...
			pbslash = strrchr(szfilenamei, '\\');
			pslash = strrchr(szfilenamei, '/');
			
			if ((pbslash != NULL) && (pslash != NULL)) {
				if (pbslash> pslash)  pdirsep=pbslash;
				else pdirsep=pslash;
				
				strcpy(ifilename, pdirsep+1);
				strcpy(szfilenametestwavo, szOutputDir);
				strcat(szfilenametestwavo,ifilename);	   	   	   	   
				
			} else if ((pbslash == NULL) && (pslash == NULL)) {

				strcpy(szfilenametestwavo, szOutputDir);	  	  
	   			strcat(szfilenametestwavo, szfilenamei);
		...
		} else strcpy(szfilenametestwavo, szfilenamei);

		//printf("%s", szfilenametestwavo);
		...
		if (g_cflAppendEnable) {
			
	        strcat(szfilenametestwavo, "_");
	        if (g_nSamplesPerSecond == SPEED_44K)
	            strcat(szfilenametestwavo, "44");
	        else if (g_nSamplesPerSecond == SPEED_22K)
	            strcat(szfilenametestwavo, "22");
	        else if (g_nSamplesPerSecond == SPEED_11K)
	            strcat(szfilenametestwavo, "11");
	        else if (g_nSamplesPerSecond == SPEED_10K)
	            strcat(szfilenametestwavo, "10");
	        else if (g_nSamplesPerSecond == SPEED_7K)
	            strcat(szfilenametestwavo, "7");
	        else if (g_nSamplesPerSecond == SPEED_3K)
	            strcat(szfilenametestwavo, "3");
